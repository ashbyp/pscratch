data="""F-|-J7F7F7F-7F7F--7-L|-L7777.FFL-|.|.FF-FF7FFFFF7--F-J7.|-FF77|-F|777-F7.F7FF|-J7|-7.FFJ77FL.7-|-|7.|.77|-F-|-F|-FL.F77.FF-77|7.7--7--7FJ--7
...
def pr(grid, sym=None):
    rows, cols = len(grid), len(grid[0])
    for r in grid:
        print([_r for _r in r])


def create_empty_grid(g):
    rows, cols = len(grid), len(grid[0])
...
def make_grid():
    ...
    for line in data.splitlines():
        g.append(['.'] + list(line) + ['.'])
    extra_row = ['.'] * len(g[0])
    g.insert(0, extra_row)
    g.append(extra_row)
    return g


def copy_grid(g):
    return [_r.copy() for _r in g]


def find_pipe(g, pipe):
    return [(x, y) for x in range(len(grid)) for y in range(len(grid[0])) if g[x][y] == pipe][0]
...
def is_valid_move(src, tar, direction):
    return direction in VALID[src] and tar in VALID.keys()


grid = make_grid()
pr(grid)
x, y = find_pipe(grid, 'S')
print('start ', x, y)

moves = 0
prev_direction = None

loop_map = create_empty_grid(grid)

while True:
    loop_map[x][y] = grid[x][y]

    if prev_direction != S and is_valid_move(grid[x][y], grid[x - 1][y], N):
        # print('N')
        x -= 1
        prev_direction = N
        moves += 1
    elif prev_direction != N and is_valid_move(grid[x][y], grid[x + 1][y], S):
        # print('S')
        x += 1
        prev_direction = S
        moves += 1
    elif prev_direction != W and is_valid_move(grid[x][y], grid[x][y + 1], E):
        # print('E')
        y += 1
        prev_direction = E
        moves += 1
    elif prev_direction != E and is_valid_move(grid[x][y], grid[x][y - 1], W):
        # print('W')
        y -= 1
        prev_direction = W
        moves += 1
...
def is_onclosed(g, row, col, nrows, ncols):
    if row == 7 and col == 8:
        print('bym')

    if g[row][col] in VALID.keys(): return False

    for r_ in range(row+1, nrows):
        if g[r_][col] in VALID.keys():
            break
    else:
        return False

    for r_ in range(row-1, -1, -1):
        if g[r_][col] in VALID.keys():
            break
    else:
        return False

    for c_ in range(col+1, ncols):
        if g[row][c_] in VALID.keys():
            break
    else:
        return False

    for c_ in range(col-1, -1, -1):
        if g[row][c_] in VALID.keys():
            return True
...
enclosed = 0
rows, cols = len(loop_map), len(loop_map[0])

enclosed_map = copy_grid(loop_map)

for r in range(rows):
    for c in range(cols):
        if loop_map[r][c] in VALID.keys():
            enclosed_map[r][c] = loop_map[r][c]
        elif is_onclosed(loop_map, r, c, rows, cols):
            enclosed_map[r][c] = '*'
        else:
            enclosed_map[r][c] = ' '
...
for r in range(rows):
    ...
    print('row', r)
    for c in range(cols):
        if enclosed_map[r][c] in ('|', 'L', 'J', 'S'):
            parity = (parity + 1) % 2
            print('parity ', parity)
        elif enclosed_map[r][c] == '*':
            if parity == 1:
                tot += 1
