pdata = """Alice would gain 54 happiness units by sitting next to Bob.
...
tdata = """Alice would gain 54 happiness units by sitting next to Bob.
...
def part2(data: str, debug: bool = False):
    ...
    for line in data.splitlines():
        words = line.split()
        scores[words[0]][words[10][:-1]] = int(words[3]) * (1 if words[2] == 'gain' else -1)

    for person, other_person in scores.items():
        scores[person]['me'] = 0
    ...
    max_happy = 0
    for perm in permutations(scores.keys()):
        happy = 0
        if debug:
            print(perm)
        for i in range(len(perm)):
            ...
            happy += scores[p1][p2] + scores[p2][p1]
            if debug:
                print('  ', p1, p2)
