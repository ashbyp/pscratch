pdata = """Sue 1: goldfish: 6, trees: 9, akitas: 0
...
"""
    ...
    for line in data.splitlines():
        ...
        counts[words[1]][words[2]] = int(words[3])
        counts[words[1]][words[4]] = int(words[5])
        counts[words[1]][words[6]] = int(words[7])
    ...
    found = []
    for sue, sue_items in counts.items():
        ct = 0
        for i, c in sue_items.items():
            if i in ('cats', 'trees'):
                if needed[i] < c:
                    ct += 1
