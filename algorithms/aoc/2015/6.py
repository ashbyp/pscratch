pdata = """turn on 489,959 through 759,964
...
tdata = """turn on 0,0 through 9,9
turn off 1,1 through 8,8
...
def part2(data: str):
    width = 999
    # width = 9
    grid = [[0 for _ in range(width + 1)] for _x in range(width + 1)]

    for ins in data.splitlines():
        tokens = ins.split()

        if tokens[1] in ('off', 'on'):
            r1, c1 = tokens[2].split(',')
            r2, c2 = tokens[4].split(',')
            p1 = (int(r1), int(c1))
            p2 = (int(r2), int(c2))
            for r in range(p1[0], p2[0] + 1):
                for c in range(p1[1], p2[1] + 1):
                    grid[r][c] = max(grid[r][c] + (1 if tokens[1] == 'on' else -1), 0)
        else:
            r1, c1 = tokens[1].split(',')
            r2, c2 = tokens[3].split(',')
            p1 = (int(r1), int(c1))
            p2 = (int(r2), int(c2))
            for r in range(p1[0], p2[0] + 1):
                for c in range(p1[1], p2[1] + 1):
...
def part1(data: str):
    width = 999
    # width = 9
    grid = [[False for _ in range(width + 1)] for _x in range(width + 1)]
