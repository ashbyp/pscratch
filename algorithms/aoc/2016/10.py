pdata = """bot 171 gives low to bot 4 and high to bot 84
...
def part2(data: str, debug: bool = False):
    ...
    while True:
        next_action = [(bot, values) for bot, values in bots.items() if len(values) == 2]
        if not next_action:
        ...
        bot = next_action[0]
        low, high = sorted(next_action[1])
        next_ins = ins[bot]
        bots[bot] = []

        if next_ins[5] == 'bot':
            bots[next_ins[6]].append(low)
        else:
            outputs[next_ins[6]].append(low)

        if next_ins[10] == 'bot':
            bots[next_ins[11]].append(high)
        else:
            outputs[next_ins[11]].append(high)
...
def part1(data: str, debug: bool = False):
    ...
    while True:
        next_action = [(bot, values) for bot, values in bots.items() if len(values) == 2][0]
        bot = next_action[0]
        low, high = sorted(next_action[1])
        next_ins = ins[bot]
        bots[bot] = []

        if sorted([low, high]) == [17, 61]:
            print('Bot is: ', bot)
            return
