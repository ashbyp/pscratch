pdata = """hqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]
...
tdata = """aaaaa-bbb-z-y-x-123[abxyz]
...
def decrypt(sector, cipher):
    text = ""
    for c in cipher:
        if c.isalpha():
            shift = ord('A' if c.isupper() else 'a')
            shifted = chr((ord(c) - shift + sector) % 26 + shift)
            text += shifted
        else:
            text += c
    return text
...
def part2():
    # print(decrypt(343, 'qzmt zixmtkozy ivhz'))
    ...
    for line in data.splitlines():
        ...
        mc = Counter(encrypted_name.replace('-', '')).most_common(50)
        smc = sorted(mc, key=cmp_to_key(cmp))
        smc = ''.join([x[0] for x in smc[:5]])

        if smc == checksum:
            print(decrypt(sector_id, encrypted_name), sector_id)
...
def part1():
    ...
    for line in data.splitlines():
        ...
        sector_id = int(match.group(2))
        checksum = match.group(3)
        mc = Counter(encrypted_name).most_common(50)
        smc = sorted(mc, key=cmp_to_key(cmp))
        smc = ''.join([x[0] for x in smc[:5]])

        if smc == checksum:
            # print('Real ',encrypted_name)
            tot += sector_id
    print(tot)
