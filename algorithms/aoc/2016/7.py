pdata = """rhamaeovmbheijj[hkwbkqzlcscwjkyjulk]ajsxfuemamuqcjccbc
...
tdata = """abba[mnop]qrst
abcd[bddb]xyyx
aaaa[qwer]tyui
ioxxoj[asdfgh]"""

tdata_p2 = """aba[bab]xyz
xyx[xyx]xyx
aaa[kek]eke
zazbz[bzb]cdb
"""
...
def supports_ssl(ip):
    ...
    for i, c in enumerate(ip):
        ...
        if len(letters) == 3:
            # abba
            aba = (letters[0] == letters[2]) and (letters[0] != letters[1])
            if aba:
                if inside:
                    found_inside.add(letters)
                else:
                    found_outside.add(letters)
            letters = letters[1:]

    for aba in found_inside:
        bab = aba[1] + aba[0] + aba[1]
        if bab in found_outside:
            return True
    return False
...
def supports_tls(ip):
    ...
    for i, c in enumerate(ip):
        ...
        if len(letters) == 4:
            # abba
            abba = (letters[0] == letters[3]) and (letters[1] == letters[2]) and (letters[0] != letters[1])
            if abba:
                if inside:
                    return False
                else:
                    found_outside_abba = True
            letters = letters[1:]
    return found_outside_abba
...
if __name__ == '__main__':
    ...
    run("Part 2 test", tdata_p2, part2, True)
