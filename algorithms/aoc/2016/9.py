...
def doit(data, debug=False):
        ...
        if not processing_marker:
            if c == '(':
                ...
                while data[index] != ')':
                    marker += data[index]
                    index += 1
                bef, aft = marker.split('x')
                len_marker = int(bef)
                repeats = int(aft)
                # print('Marker ', len_marker, 'x', repeats)
                index += 1
            ...
            for _ in range(repeats):
                output_str += data[index:index + len_marker]
            if '(' not in output_str:
                output += (len_marker * repeats)
                index += len_marker
            else:
                pass
            processing_marker = False
    ...
    if debug:
        print(output_str)
    print('Input len ', len(data), ' Output len ', output)
