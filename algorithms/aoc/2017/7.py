...
def part2(data: str, debug: bool = False):
    ...
    for line in data.splitlines():
        ...
        if '->' in line:
            bef, aft = line.split('>')
            node, blah, _ = bef.split()
            weight = int(blah.replace('(', '').replace(')', ''))
            others = aft.replace(' ', '').split(',')
        else:
            node, blah = line.split()
            weight = int(blah.replace('(', '').replace(')', ''))
            # print(weight)
    ...
    if data == tdata:
        ...
        for n in graph['nzeqmqi']:
            ...
            print('Root: ', n, s)
            nodes.clear()
    correction = -8 if data == tdata else -9

    if data == tdata:
        for W in weights.keys():
            print(f'Change {W} from {weights[W]} to {weights[W] + correction}')
            rem = weights[W]
            weights[W] = weights[W] + correction
            for n in graph['tknk']:
                search(n)
                w = [weights[x] for x in nodes]
                s = 0
...
def part1(data: str, debug: bool = False):
    ...
    nodes_with_ancestors = set(n for v in graph.values() for n in v)
    print(nodes.difference(nodes_with_ancestors))
