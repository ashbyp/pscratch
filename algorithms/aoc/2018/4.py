pdata = """[1518-02-28 00:47] falls asleep
...
tdata = """[1518-11-01 00:00] Guard #10 begins shift
...
class Guard:
    ...
    def add(self, day, start_min, end_min):
        times = self.days[day]
        for m in range(start_min, end_min):
            times.append(m)

    def total_asleep(self):
        return sum(len(x) for x in self.days.values())

    def most_common_min(self):
        all_mins = []
        for v in self.days.values():
            all_mins.extend(v)
        c = Counter(all_mins).most_common(1)
        # print('Guard', self.gid, c)
        if len(c):
            return c[0][0]
        return None
...
def part2(data):
    ...
    for line in sorted(data.splitlines()):
        ...
        if what == 'up':
            guards[cur_guard].add(day, int(start_min), int(m))
        elif what == 'asleep':
            start_min = m
        elif what.startswith('#'):
            cur_guard = int(what[1:])
            if cur_guard not in guards:
                guards[cur_guard] = Guard(cur_guard)
        else:
            raise ValueError(what)

    mmmc = 0
    ans = 0
    for _, g in guards.items():
        mmc = g.max_min_count()
        if mmc and mmc >= mmmc:
            ans = g.gid * g.most_common_min()
            mmmc = mmc
    print(ans)
...
def part1(data):
    ...
    ans = 0
    for _, g in guards.items():
        if g.total_asleep() > max_asleep:
            max_asleep = g.total_asleep()
            ans = g.gid * g.most_common_min()
    print(ans)
