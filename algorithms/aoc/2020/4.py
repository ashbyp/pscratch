...
tdata = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
def is_valid(attr, value):
    ...
    if attr == 'hgt':
        if value.endswith('in'):
            ...
            return False
        return lower <= int(value[0:-2]) <= upper
    if attr == 'hcl':
    ...
    for passport in data.split('\n\n'):
        ...
        for line in passport.splitlines():
            ...
            for i in range(0, len(d), 2):
                if is_valid(d[i], d[i + 1]):
                    attrs.add(d[i])

        if len(attrs.intersection(mandatory)) == num_mandatory:
            # print(passport)
            valid += 1
    print(valid)
