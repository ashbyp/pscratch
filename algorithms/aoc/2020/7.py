pdata = """bright gray bags contain 2 bright gold bags, 5 dull lavender bags.
...
tdata="""shiny gold bags contain 2 dark red bags.
...
def part2(data: str, debug: bool = False):
    ...
    for line in data.splitlines():
        stuff = line.split('contain')
        bag = ''.join(stuff[0].split(' ')[0:2])
        if "no other" in stuff[1]:
            bags[bag] = []
        else:
            for s in stuff[1].split(','):
                blah = s.split(' ')
                bags[bag].append((''.join(blah[2:4]), int(blah[1])))

    def zap(_bag, mult=1):
        if not bags[_bag]:
            return 0
        return sum(b[1] * mult for b in bags[_bag]) + sum(zap(b[0], mult * b[1]) for b in bags[_bag])
...
def part1(data: str, debug: bool = False):
    ...
    for line in data.splitlines():
        ...
        if "no other" in stuff[1]:
            ...
            for s in stuff[1].split(','):
                blah = s.split(' ')
                bags[bag].append(''.join(blah[2:4]))

    def _has(_bag):
        if not bags[_bag]:
            return False
        if 'shinygold' in bags[_bag]:
            return True
        return any([_has(b) for b in bags[_bag]])
