...
def get_stacks(block: str) -> dict[list]:
    rows = [l for l in block.splitlines()]
    stacks = {}
    for col, stack_name in enumerate(rows[-1]):
        if stack_name != ' ':
            stacks[stack_name] = []
            for r in range(len(rows) - 2, -1, -1):
                if rows[r][col] != ' ':
                    stacks[stack_name].append(rows[r][col])
    return stacks


def part2(data: str):
    blocks = data.split('\n\n')
    stacks = get_stacks(blocks[0])

    for ins in blocks[1].splitlines():
        _, how_many, _, src, _, target = ins.split(' ')
        crates = []
    ...
    code = [x.pop() for x in stacks.values()]
    print(''.join(code))
